...
class RLStrategy(Strategy):
    ...
    def store(self, state: MarketState, action: Action, reward: float,
        ...
        saved data in run.py), they take precedence over self._last_* which only
        reflects the MOST RECENT act() call and may belong to a different cid.
        """
        self.reward_count += 1
    ...
    def update(self) -> Optional[Dict[str, float]]:
        ...
        # Advantage stats (pre-normalization: use raw advantages before std-norm)
        raw_adv = returns - old_values
        avg_advantage = float(np.mean(raw_adv))
        advantage_std = float(np.std(raw_adv))

        # Critic value prediction stats
