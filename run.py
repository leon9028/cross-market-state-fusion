    ...
    def _execute_live_order(
        ...
        if pos.size > 0 or pos.shares > 0:
            ...
            if action.is_sell and pos.side == "UP":
                ...
                print(f"    [LIVE] SUBMIT CLOSE UP {pos.asset} {side} {pos.shares} @ {order_price}")
                create_and_submit_order(
                    self.clob_client, token_id, side, order_price, pos.shares, order_type=OrderType.FOK
                )
            ...
            # Strategy says BUY (buy UP) and we hold DOWN → close DOWN position by selling token_down
                ...
                print(f"    [LIVE] SUBMIT CLOSE DOWN {pos.asset} {side} {pos.shares} @ {order_price}")
                create_and_submit_order(
                    self.clob_client, token_id, side, order_price, pos.shares, order_type=OrderType.FOK
                )
                self._pending_orders.add(token_id)
